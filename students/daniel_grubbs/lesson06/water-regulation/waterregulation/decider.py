...
class Decider(object):
    ...
    def __init__(self, target_height, margin):
        ...
        :param margin: the margin of liquid above and below the target
                height for which the pump should not turn on. Ex: .05
    ...
    def decide(self, current_height, current_action, actions):
        ...
        """
        top_height = self.target_height + (self.target_height * self.margin)
        bottom_height = self.target_height - (self.target_height * self.margin)
        # PUMP_IN = 1
        # PUMP_OFF = 0
        # PUMP_OUT = -1

        # Behavior 1
        # tested with the above PUMP_IN, PUMP_OFF, PUMP_OUT but removed after
        # performing the pylint testing
        if (current_action == 0) and (current_height < bottom_height):
            return actions['PUMP_IN']
        if (current_action == 0) and (current_height < bottom_height):
            return actions['PUMP_IN']
        # Behavior 2
        if (current_action == 0) and (current_height > top_height):
            return actions['PUMP_OUT']
        # Behavior 3
        if (current_action == 0) and (bottom_height <=
        ...
        if (current_action == -1 and current_height < self.target_height):
            ...
            return actions['PUMP_OUT']
        # else:
        #     raise MyException('There was no matching behavior.')
