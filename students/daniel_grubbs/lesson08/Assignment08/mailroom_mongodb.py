...
def gen_letter(donor):
    ...
    return dedent('''Dear {0:s},
          ...
          It will be put to very good use.
                         Sincerely,
                            -The Team
          '''.format(donor['donor'], donor['donations'][-1]))
...
def send_thank_you():
    ...
    donor_db.update({'donor': name}, {'$push': {'donations': amount}})

    # print out letter for donor/donation
    cursor = donor_db.find({'donor': name})
    # print(cursor)
    for d in cursor:
        print(gen_letter(d))
...
def remove_donor_from_db():
    ...
    listing = donor_db.find({})
    for l in listing:
        print(l['donor'])
    donor_name = input('Which donor do you wish to remove? ')
    cursor = donor_name.find({'donor': {'$eq': donor_name}})
    # print(cursor)
    for d in cursor:
        print(f"Name: {d['donor']} Donation: {d['donations']}")

    donor.remove({"donor": {"$eq": donor}}, {"donations": {"$eq": donation}})

    cursor = donor.find({'donor': {'$eq': donor}})
    for d in cursor:
        print(f"Name: {d['donor']} Donation: {d['donations']}")
