...
def run_example(furniture_items):
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        log.info('just the red products')
        query_red = {'color': 'Red'}
        cursor_red = furniture.find(query_red)
        print('Query for red products: ')
        for c in cursor_red:
            print(f"Color: {c['color']} Product type: {c['product']} Description: {c['description']}")


        log.info('just the couches')
        query_couch = {'color': 'Red'}
        cursor_couch = furniture.find(query_couch)
        print('Query for couch products: ')
        for couch in cursor_couch:
            print(f"Product type: {couch['product']} Color: {couch['color']} Description: {couch['description']}")
