...
def run_example():
    ...
    with driver.session() as session:
        ...
        cyph = """
          ...
          """
        result = session.run(cyph)
        print('The type result is ',type(result))
        print("\nMarie's friends are:")
        for rec in result:
            for friend in rec.values():
                print(friend['first_name'], friend['last_name'])
# ////////////////////////////////////////////////////////////////////////////////////
        names = [('Donald', 'Trump'), ('Kevin', 'Spacy'), ('Dennis', 'Muilenburg')]
        colors = ['Gold', 'Red', 'Blue', 'Green']

        log.info('Adding a few more people nodes')
        for first, last in names:
            cyph = "CREATE (n:Person {first_name:'%s', last_name: '%s'})" % (
                first, last)
            session.run(cyph)

        log.info('Adding color nodes')
        for color in colors:
        ...
        log.info("Setting up favorite colors")
        i = 0 
        for first, last in names:
            cyph = "MATCH (p:Person {first_name: '%s', last_name: '%s'})" % (first, last)
            cyph += "CREATE (p)-[fc:FC]->(c:Color {color: '%s'})" % (colors[i])
            cyph += "RETURN p as person, p.color as color"
            i += 1 
            result = session.run(cyph)

        cyph = """
                 MATCH (p:Person {first_name:'Donald', last_name:'Trump'})
                 CREATE (p1)-[fav:FAV]->(c:Color {color:'Green'})
                 RETURN p1
                 """
        session.run(cyph)