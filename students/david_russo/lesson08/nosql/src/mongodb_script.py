...
def run_example(furniture_items):
    ...
    with login_database.login_mongodb_cloud() as client:
        ...
        furniture.remove({"product type": {"$eq": "Couch"}, "color": {"$eq": "Blue"}})
        ...
        log.info('Step 8: Find just the red products')
        cursor = furniture.find({'color': 'Red'}).sort('color')
        for doc in cursor:
            print(f"Color: {doc['color']} product name: {doc['product type']}")

        log.info('Step 9: Find just the couches')
        cursor = furniture.find({'product type': 'Couch'}).sort('product type')
        for doc in cursor:
            print(f"Color: {doc['color']} product name: {doc['product type']}")  
