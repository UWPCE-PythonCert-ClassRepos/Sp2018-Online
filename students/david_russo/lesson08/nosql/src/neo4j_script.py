...
def run_example():
    ...
    with driver.session() as session:
        ...
        log.info('Step 2: Adding a few Color nodes too')
        ...
        log.info("Step 3: Print all colors in the DB")
        cyph = """MATCH (c:Color)
        ...
        log.info('Step 4: Create some color relationshios')
        log.info("Bob Jones like green and yellow")

        for color in ['green', 'yellow']:
            cyph = """
              MATCH (p1:Person {first_name:'Bob', last_name:'Jones'})
              CREATE (p1)-[fav_col:FAV_COL]->(c1:Color {color:'%s'})
              RETURN p1
            """ % (color)
            session.run(cyph)
        ...
        log.info("Step 5: Find all of Bob's favorite colors")
        cyph = """
        MATCH (bob {first_name:'Bob', last_name:'Jones'})
              -[:FAV_COL]->(bobColor)
        RETURN bobColor
        ...
        log.info('Step 6: Create some color relationshios')
        log.info("Marie Curie like yellow and white")
        ...
        log.info("Step 6: Find all of Marie's favorite colors")
        cyph = """
        ...
        """
        result = session.run(cyph)
        print("Marie's favorite colors are:")
        for rec in result:
            for color in rec.values():
                print(color['color'])
