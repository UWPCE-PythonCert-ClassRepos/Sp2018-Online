...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        # PUMP_OFF = 0
        # PUMP_OUT = -1
        lower_margin = self.target_height * (1 - self.margin)
        upper_margin = self.target_height * (1 + self.margin)

        if current_action == 0 and current_height < lower_margin:
            return actions['PUMP_IN']
        elif current_action == 0 and current_height > upper_margin:
            return actions['PUMP_OUT']
        elif (current_action == 0 and current_height >= lower_margin and
              current_height <= upper_margin):
            return actions['PUMP_OFF']
        elif (current_action == 1 and current_height > self.target_height or
              current_action == -1 and current_height < self.target_height):
            return actions['PUMP_OFF']
