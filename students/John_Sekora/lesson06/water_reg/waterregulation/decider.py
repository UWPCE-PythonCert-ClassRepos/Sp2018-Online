...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        current_height_above = self.current_height * (1 + self.margin)
        current_height_below = self.current_height * (1 - self.margin)


        if self.current_action == 'PUMP_OFF' and \
                (current_height_below > self.target_height):
            action = actions['PUMP_IN']

        if self.current_action == 'PUMP_OFF' and \
                (current_height_above < self.target_height):
            action = actions['PUMP_OUT']

        if self.current_action == 'PUMP_OFF' and \
                (current_height_below <= self.target_height <=
                     current_height_above):
        ...
        if self.current_action == 'PUMP_OUT' \
                and (current_height_below >= self.target_height):
            action = actions['PUMP_OUT']
