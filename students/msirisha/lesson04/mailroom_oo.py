...
class Donors(js.JsonSaveable):
    ...
    def __init__(self, donors_list=None):
        ...
        if not self.donors_list:
            self.donors_list = js.List()
        else:
    ...
    def load_from_file(self, temp_file="donors_db.json"):
        try:
            with open(temp_file, 'r') as tempfile:
                file_content = js.from_json(tempfile)
            return file_content
        except IOError:
            #return self.load_donors_list()
            return Donors([Donor(donor, donation) for donor, donation in {"sai emani": [20.23, 30.456, 50.786],
                   "sirisha marthy": [67.89, 45.89],
                   "ani emani": [12.789, 5.456],
                   "charles dickens": [15.89, 89.20, 345.67],
                   "mark twain": [678.986]}])
    def load(self):
        print(self.load_from_file())
    ...
    def save(self):
        json_data = self.load()
        for donor in self.donors_list:
            if donor.name not in json_data:
                json_data[donor.name] = donor.donations

        try:
            with open(temp_file, 'r') as temp_file:
                file_content = js.to_json(json_data)
        except IOError:
            return file_content
...
if __name__ == "__main__":
    ...
    menu_fns = [
       ...
       # ('load donors list', donors_obj.load_donors_list, None),
        ('load donors list', donors_obj.load, None),
        #('save donors list', donors_obj.save_donors_list, None),
        ('save donors list', donors_obj.save, None),
        ('quit', sys.exit, None)
    ]
    print("came here")
