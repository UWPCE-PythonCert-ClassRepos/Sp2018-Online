...
class Controller(object):
    ...
    def tick(self):
        ...
        # doneTODO: Implement the above-defined behaviors
        liquid_h = self.sensor.measure()
        pump_state = self.pump.get_state()
        decision_state = self.decider.decide(current_height=liquid_h, current_action=pump_state, actions=self.actions)
        # pump.set_state returns True for acknowledged state, false for not
        if self.pump.set_state(decision_state):
            return True
        else:
            return False
