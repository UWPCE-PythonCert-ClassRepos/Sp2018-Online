...
class Decider(object):
    ...
    def decide(self, current_height, current_action, actions):
        ...
        # doneTODO: Implement the properties of this method described above.
        ubound_h = self.target_height + self.target_height * self.margin
        lbound_h = self.target_height - self.target_height * self.margin
        # current_action is a call to pump.get_state that returns 1 for PUMP_IN, 0 for PUMP_OFF and -1 for PUMP_OUT
        if current_action == 0:
            if current_height < lbound_h:
                return actions['PUMP_IN']
            if current_height > ubound_h:
                return actions['PUMP_OUT']
            if current_height >= lbound_h and current_height <= ubound_h:
