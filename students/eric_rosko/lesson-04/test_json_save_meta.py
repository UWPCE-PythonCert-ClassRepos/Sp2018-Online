...
def test_with_helper_class():

    class DictSaver(js.JsonSaveable):
        dict = js.Dict()
    ...
    ds2 = DictSaver.from_json_dict(saved)
...
def test_with_helper_class_and_file_read_write():

    os.remove("temp.json")
    class DictSaver(js.JsonSaveable):
        _dictionary = js.Dict()
        def __init__(self, dict2 = None):
            if dict2 is not None:
                self._dictionary = dict2
    ...
    ds = DictSaver(d)
    saved = ds.to_json_compat()
    contents_to_save = str(saved)
    print('contents_to_save\n', contents_to_save, '\n')
    with open("temp.json", 'w') as tempfile:
        tempfile.write(contents_to_save)
