...
def get_sample_data():
    ...
    This now pulls that list from the database.
    """
    ...
    output_table = []
    query1 = Donor_Person.select()
    for donor_person in query1:
        logging.info("Compiling data for " + str(donor_person.person_name))
        donation_table = []
        query2 = Donation.select().where(Donation.gift_donor ==
                                         donor_person.person_name).\
            order_by(Donation.gift_date)
        for donation in query2:
            donation_table.append(float(donation.gift_value))
        output_table.append(Donor(donor_person.person_name, donation_table))
...
class DonorDB():
    """
    encapsulation of the entire database of donors and data associated with
    them.
    """
