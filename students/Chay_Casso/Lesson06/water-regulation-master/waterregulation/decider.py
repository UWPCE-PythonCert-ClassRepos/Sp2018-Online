...
class Decider(object):
    ...
    def __init__(self, target_height, margin):
        ...
        self.margin = margin
        self.low_height = target_height * (1 - margin)
        self.high_height = target_height * (1 + margin)
    ...
    5. If the pump is performing PUMP_OUT and the height is below the target
       ...
       shall remain at PUMP_OUT.
        ...
        if current_action == "PUMP_OFF" and current_height < self.low_height:
            return actions["PUMP_IN"]
        elif current_action == "PUMP_OFF" and current_height > \
                self.high_height:
            return actions["PUMP_OUT"]
        elif current_action == "PUMP_OFF":
            return actions["PUMP_OFF"]
        elif current_action == "PUMP_IN" and current_height > \
                self.target_height:
            return actions["PUMP_OFF"]
        elif current_action == "PUMP_IN":
            return actions["PUMP_IN"]
        elif current_action == "PUMP_OUT" and current_height < \
                self.target_height:
            return actions["PUMP_OFF"]
        return actions["PUMP_OUT"]
