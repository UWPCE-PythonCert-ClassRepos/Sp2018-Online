...
def run_example(furniture_items):
    ...
    def run_csv():
        ...
        log.info("And now here's the homework:")
        songdata = [
            ('True Faith', 'New Order', 'Substance', '1987', '4:30'),
            ('Love to Hate You', 'Erasure', 'Hits! The Very Best of Erasure', '1991', '3:56'),
            ('Things Can Only Get Better', 'Howard Jones', 'Dream Into Action', '1985', '3:57'),
            ('Everybody Wants to Rule the World', 'Tears for Fears', 'Songs from the Big Chair', '1985', '4:11'),
            ('Enjoy the Silence', 'Depeche Mode', 'Violator', '1990', '4:16'),
            ('Only Happy When It Rains', 'Garbage', 'Garbage', '1995', '3:56'),
            ('Slide', 'The Goo Goo Dolls', 'Dizzy Up the Girl', '1998', '3:32'),
            ('Porcelain', 'Moby', 'Play', '2000', '4:01'),
            ('Interstate Love Song', 'Stone Temple Pilots', 'Thank You', '1994', '3:14'),
            ('Opposites Attract', 'Paula Abdul', 'Forever Your Girl', '1989', '3:45')
        ]

        log.info("Writing the csv file")
        with open('../data/favsongs.csv', 'w') as songs:
            songwriter = csv.writer(songs)
            songwriter.writerow(songdata)

        log.info("Read csv file back")
        with open('../data/favsongs.csv', 'r') as songs:
            song_reader = csv.reader(songs, delimiter=',', quotechar='"')
